...
public static class GlobalStatic
{
    ...
    public static async Task GenerateHTMLListIndex(GeFeSLEDb db)
    {
        ...
        else
        {
            ...
            {
                var itemCount = await db.Items.CountAsync(x => x.ListId == list.Id && x.Visible && !x.IsDeleted);
    ...
    public static async Task GeneratePageFooter(StringBuilder sb)
    {
        ...
        sb.AppendLine($"<div class=\"byline\">The owner of this instance is: {WebUtility.HtmlEncode(GlobalConfig.owner ?? string.Empty)}</div>");
